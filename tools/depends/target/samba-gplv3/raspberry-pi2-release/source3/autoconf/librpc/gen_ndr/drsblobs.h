...
struct package_PrimaryKerberosKey3 {
	...
	DATA_BLOB *value;/* [subcontext(0),relative,flag(LIBNDR_FLAG_REMAINING),subcontext_size(value_len)] */
};
...
struct package_PrimaryKerberosKey4 {
	...
	uint32_t keytype;
	uint32_t value_len;/* [value((value?value->length:0))] */
	DATA_BLOB *value;/* [relative,flag(LIBNDR_FLAG_REMAINING),subcontext_size(value_len),subcontext(0)] */
};
...
struct ExtendedErrorUString {
	uint16_t __size;
	const char *string;/* [size_is(__size),unique,charset(UTF16)] */
};
